...
原任务，不会生成第二条投递记录。同一告警恢复后再次触发时会进入新的生命周期，
生成新的投递任务，不会被历史成功任务错误去重。

`GET /healthz` 会返回投递 Worker 是否启用、是否运行、待处理数量、死信数量和最近一次
异常类型。Worker 对任务领取、回调执行和状态写回异常进行隔离与有限退避，不会因单次
未预期异常永久退出。
