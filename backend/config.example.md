...
When the database section is missing, KuberPilot uses local SQLite at `backend/db.sqlite3`.
...
When the cache section is missing, KuberPilot uses Django in-memory cache.
