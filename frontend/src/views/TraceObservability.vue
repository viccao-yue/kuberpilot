<template>
  <div class="observability-page" :class="[{ 'is-embedded': embeddedMode }, { 'workbench-page-shell': !embeddedMode }]">
    ...
    <ObservabilityRouteTabs
      v-if="!embeddedMode && !topologyOnly"
      :group="activeTraceTab === 'datasources' ? 'datasources' : 'query'"
    />

    <template v-if="activeTraceTab === 'traces' && canViewTrace">
    <div v-if="!topologyOnly" class="audit-grid trace-overview-grid">
      <div class="audit-card audit-card--inline">
        <div class="stat-label">当前数据源</div>
        <div class="stat-value trace-overview-value">{{ selectedTracingDataSource?.name || '--' }}</div>
      </div>
      <div class="audit-card audit-card--inline">
        <div class="stat-label">命中 Trace</div>
        <div class="stat-value">{{ searchSummary.match_count || displayTraces.length }}</div>
      </div>
      <div class="audit-card audit-card--inline" :class="{ 'audit-card--warning': topologyHighlights.length > 0 }">
        <div class="stat-label">拓扑节点</div>
        <div class="stat-value">{{ focusedTopology.node_count || 0 }}</div>
      </div>
      <div class="audit-card audit-card--inline" :class="{ 'audit-card--danger': Boolean(selectedTraceId && traceDetail?.error_count) }">
        <div class="stat-label">详情状态</div>
        <div class="stat-value">{{ loading.detail ? '加载中' : selectedTraceId ? '已选中' : '待选择' }}</div>
      </div>
    </div>

    <section v-if="!topologyOnly" class="workbench-inline-tip--panel trace-context-tip">
      <div class="tip-panel-head">
        <strong>Trace 查询上下文</strong>
        <span>把 Provider、服务、时间窗和排序条件前置展示，进入详情前先快速确认当前检索语境。</span>
      </div>
      <div class="tip-panel-list">
        <div class="tip-panel-item">Provider：{{ selectedProviderMeta?.provider_name || '--' }}</div>
        <div class="tip-panel-item">时间窗口：{{ timeRangeSummary }}</div>
        <div class="tip-panel-item">排序方式：{{ sortLabel }}，状态 {{ filters.traceState === 'ALL' ? '全部' : filters.traceState === 'ERROR' ? '错误' : '正常' }}</div>
        <div class="tip-panel-item">当前选中 {{ selectedTraceId ? '1 条 Trace' : '未选 Trace' }}，详情 Span {{ traceDetail?.span_count || 0 }} 个</div>
      </div>
    </section>
