  ...
  >
    ...
    <template #stats>
      <div v-if="currentTab" class="audit-grid logs-overview-grid">
        <div class="audit-card audit-card--inline">
          ...
          <div class="stat-value logs-overview-value">{{ currentDataSource?.name || '--' }}</div>
        </div>
        <div class="audit-card audit-card--inline" :class="{ 'audit-card--warning': errorCount > 0 }">
          <div class="stat-label">返回日志</div>
          <div class="stat-value">{{ currentResults.logs.length }}</div>
        </div>
        <div class="audit-card audit-card--inline">
          <div class="stat-label">总匹配量</div>
          <div class="stat-value">{{ currentResults.total || 0 }}</div>
        </div>
        <div class="audit-card audit-card--inline" :class="{ 'audit-card--danger': Boolean(currentTab.errorMessage) }">
    ...
    <template #hint>
      <section v-if="currentTab" class="workbench-inline-tip--panel logs-context-tip">
        <div class="tip-panel-head">
          ...
          <span>先看数据源与时间窗口，再进入条件编排和结果明细，减少在多数据源之间切换时的认知跳跃。</span>
        </div>
        <div class="tip-panel-list">
          <div class="tip-panel-item">当前 Provider：{{ providerLabel(activeProvider) || '--' }}</div>
          <div class="tip-panel-item">时间范围：{{ formatTimeRangeSummary(currentTab.timeRange) }}</div>
          <div class="tip-panel-item">返回 {{ currentResults.logs.length }} 条，错误 {{ errorCount }} 条</div>
    ...
    <template v-else>
      ...
      <section v-if="currentTab" class="panel compact-panel">
        <div class="panel-head slim-head">
          <h3>查询结果</h3>
          <div class="result-tags result-tags--compact">
