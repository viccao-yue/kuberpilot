<template>
  <div class="observability-page" :class="[{ 'is-immersive': fullscreenVisible }, { 'workbench-page-shell': !fullscreenVisible }]">
    ...
    <template v-else>
      ...
      <div class="audit-grid grafana-overview-grid">
        <div class="audit-card audit-card--inline">
          <div class="stat-label">目录总数</div>
          <div class="stat-value">{{ folderCount }}</div>
        </div>
        <div class="audit-card audit-card--inline" :class="{ 'audit-card--success': configuredDashboardCount > 0 }">
          <div class="stat-label">已配置看板</div>
          <div class="stat-value">{{ configuredDashboardCount }}</div>
        </div>
        <div class="audit-card audit-card--inline">
          <div class="stat-label">标签数量</div>
          <div class="stat-value">{{ tagOptions.length }}</div>
        </div>
        <div class="audit-card audit-card--inline">
          <div class="stat-label">当前选中</div>
          <div class="stat-value grafana-overview-value">{{ selectedDashboard?.title || '--' }}</div>
        </div>
      </div>

      <section class="workbench-inline-tip--panel grafana-context-tip">
        <div class="tip-panel-head">
          <strong>看板上下文</strong>
          <span>先看目录、标签和已配置数量，再进入目录树与看板清单，减少筛选与展开过程中来回跳读。</span>
        </div>
        <div class="tip-panel-list">
          <div class="tip-panel-item">目录 {{ folderCount }} 个，已配置 URL 的看板 {{ configuredDashboardCount }} 个</div>
          <div class="tip-panel-item">当前筛选：{{ hasActiveFilters ? `关键词 ${filters.keyword || '未填'} / 标签 ${filters.tag || '全部'}` : '未启用筛选' }}</div>
          <div class="tip-panel-item">当前目录：{{ selectedDashboard?.folder || '未分类' }}</div>
          <div class="tip-panel-item">当前看板：{{ selectedDashboard?.title || '未选择' }}</div>
        </div>
      </section>
      ...
      <section class="panel">
        <div class="section-head section-head--list">
          <div class="section-title-block">
            ...
            <span class="section-title-summary">统一维护目录层级、标签与看板跳转入口。</span>
          </div>
        ...
        <div class="list-toolbar">
          ...
          <div class="list-toolbar__summary">
            ...
            <span>{{ configuredDashboardCount }} 个已配置</span>
...
.grafana-overview-grid {
  grid-template-columns: repeat(4, minmax(0, 1fr));
}

.grafana-overview-value {
