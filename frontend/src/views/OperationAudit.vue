    ...
    <div class="workbench-card operation-audit-card">
      ...
      <div v-if="loading" class="console-skeleton">
        <div class="console-skeleton-card console-skeleton-table">
          <div v-for="n in 5" :key="`audit-loading-${n}`" class="console-skeleton-row console-skeleton-row--audit">
      ...
      <el-table v-else :data="audits" stripe style="width: 100%">
        ...
        <el-table-column label="动作" width="156">
          <template #default="{ row }">
            <span class="action-cell" :title="row.action || '-'">{{ row.action || '-' }}</span>
          </template>
        </el-table-column>
...
.operation-audit-stats .audit-card--inline {
  min-height: 0;
  padding: 10px 12px;
}

.operation-audit-stats .stat-label {
  font-size: 12px;
}

.operation-audit-stats .stat-value {
  font-size: 22px;
}
