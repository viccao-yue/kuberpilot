  ...
  >
    ...
    <template #stats>
      ...
      <div class="audit-grid event-overview-grid">
        <div class="audit-card audit-card--inline">
          ...
          <div class="stat-value event-overview-value">{{ scope.environment ? environmentDisplayName(scope.environment) : '--' }}</div>
        </div>
        <div class="audit-card audit-card--inline">
          <div class="stat-label">命中事件</div>
          <div class="stat-value">{{ filteredEvents.length }}</div>
        </div>
        <div class="audit-card audit-card--inline" :class="{ 'audit-card--danger': filteredFailedCount > 0 }">
          <div class="stat-label">失败事件</div>
          <div class="stat-value">{{ filteredFailedCount }}</div>
        </div>
        <div class="audit-card audit-card--inline">
          <div class="stat-label">活跃分类</div>
          <div class="stat-value">{{ activeCategoryCount }}</div>
        </div>
      </div>

      <section class="workbench-inline-tip--panel event-context-tip">
        <div class="tip-panel-head">
          <strong>事件排障上下文</strong>
          <span>先锁定环境、时间窗与结果态，再查看时间线和分类列表，减少在多来源事件之间来回跳读。</span>
        </div>
        <div class="tip-panel-list">
          <div class="tip-panel-item">时间窗口：{{ formatTransactionWindow }}</div>
          <div class="tip-panel-item">事件源 {{ sourceOptions.length }} 个，当前分类 {{ timelineCategoryLabel }}</div>
          <div class="tip-panel-item">已启用 {{ activeFilterChips.length }} 个附加条件</div>
    ...
    <section class="query-panel panel">
      <div class="query-head">
        ...
        <div class="query-head-meta">
          <span>{{ activeScopeSummary }}</span>
          <span>当前 {{ selectedSourceLabel }}</span>
          <span>{{ activeFilterChips.length }} 个附加条件</span>
        </div>
      ...
      <div class="query-spotlight-grid">
        <div class="query-spotlight-card" :class="{ 'is-highlight': Boolean(scope.environment) }">
          <span>环境上下文</span>
          <strong>{{ scope.environment ? environmentDisplayName(scope.environment) : '请选择环境' }}</strong>
          <em>{{ scope.system_name || '未选系统' }} / {{ scope.application || '未选服务' }}</em>
        </div>
        <div class="query-spotlight-card">
          <span>事件来源</span>
          <strong>{{ selectedSourceLabel }}</strong>
          <em>{{ sourceOptions.length }} 个来源可筛选</em>
        </div>
        <div class="query-spotlight-card">
          <span>时间窗口</span>
          <strong>{{ formatTransactionWindow }}</strong>
          <em>拖拽时间轴可快速缩小范围</em>
        </div>
        <div class="query-spotlight-card" :class="{ 'is-warning': Boolean(keyword.trim()) || Boolean(resultFilter) }">
          <span>结果与关键字</span>
          <strong>{{ resultFilter ? resultLabel({ result: resultFilter }) : '全部结果' }}</strong>
          <em>{{ keyword.trim() || '未设置关键字过滤' }}</em>
        </div>
      </div>
      ...
      <div class="query-summary-caption">
        <span>已应用条件</span>
        <em>点击标签可移除对应筛选，环境与时间窗口保持锁定。</em>
      </div>
    ...
    <section class="panel category-panel">
      ...
      <div class="category-meta-strip">
        <div class="category-meta-item">
          <span>当前分类</span>
          <strong>{{ activeCategorySection.label }}</strong>
        </div>
        <div class="category-meta-item">
          <span>筛选结果</span>
          <strong>{{ activeCategorySection.events.length }} / {{ activeCategorySection.total }}</strong>
        </div>
        <div class="category-meta-item" :class="{ 'is-danger': activeCategorySection.failed > 0 }">
          <span>失败事件</span>
          <strong>{{ activeCategorySection.failed }}</strong>
        </div>
        <div class="category-meta-item">
          <span>分类过滤</span>
          <strong>{{ activeCategoryFilterSummary }}</strong>
        </div>
      </div>
...
const filteredFailedCount = computed(() => filteredEvents.value.filter(item => item.result === 'failed').length)
...
const activeCategoryCount = computed(() => categorySections.value.filter(item => item.total > 0).length)
...
const activeCategoryFilterSummary = computed(() => {
  const filters = categoryFilters[activeCategoryTab.value] || {}
  if (activeCategoryTab.value === 'application_release') {
    const values = [
      filters.service ? `服务 ${filters.service}` : '',
      filters.version ? `版本 ${filters.version}` : '',
      filters.action ? `动作 ${releaseAction({ action: filters.action })}` : '',
    ].filter(Boolean)
    return values.join(' / ') || '未添加分类过滤'
  }
  return filters.keyword ? `关键字 ${filters.keyword}` : '未添加分类过滤'
})
...
const selectedSourceLabel = computed(() => {
  if (!eventSourceCode.value) return '全部事件源'
  const source = sourceOptions.value.find(item => item.code === eventSourceCode.value)
  return source?.name || eventSourceCode.value
})
const activeScopeSummary = computed(() => {
  return [
    scope.environment ? environmentDisplayName(scope.environment) : '',
    scope.system_name,
    scope.application,
...
.query-head-meta::-webkit-scrollbar {
  display: none;
}

.query-head-meta span {
  flex: 0 0 auto;
  min-height: 24px;
  padding: 0 8px;
  border-radius: 999px;
...
.query-spotlight-grid {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 8px;
}

.query-spotlight-card {
  min-width: 0;
  padding: 9px 10px;
  border: 1px solid var(--border-color);
  border-radius: var(--card-radius, 6px);
  ...
  flex-direction: column;
  gap: 3px;
  box-shadow: inset 0 -1px 0 rgba(241, 245, 249, 0.9);
}

.query-spotlight-card span,
.query-spotlight-card em {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
...
.query-spotlight-card.is-highlight {
  border-color: rgba(0, 82, 217, 0.18);
  background: #ffffff;
}

.query-spotlight-card.is-warning {
...
.query-summary-caption span {
  color: var(--text-primary);
  font-size: 12px;
  font-weight: 700;
}

.query-summary-caption em {
  color: var(--text-muted);
  font-size: 11px;
  font-style: normal;
}
...
.category-meta-item.is-danger {
  background: #fff7f7;
  border-color: rgba(245, 74, 69, 0.14);
}
...
.axis-selection {
  ...
  box-shadow: inset 0 0 0 1px rgba(0, 82, 217, 0.28);
  pointer-events: none;
}
