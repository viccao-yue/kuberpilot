<template>
  <div class="fade-in workbench-page-shell k8s-page-shell">
    <section class="hero panel k8s-hero">
      <div class="k8s-hero-copy">
        <span class="k8s-hero-eyebrow">Container Console</span>
        <div class="release-hero-title-row release-hero-title-inline">
          <span class="release-header-icon k8s-header-icon"><el-icon><Monitor /></el-icon></span>
          <div class="k8s-hero-title-block">
            <h2>K8s 集群</h2>
            <p class="subtitle inline-subtitle k8s-hero-desc">统一查看集群、工作负载、网络、存储与配置资源。</p>
          </div>
        </div>
      </div>
      <div class="k8s-hero-meta">
        <span class="k8s-hero-chip">当前视图 · {{ activeTabLabel }}</span>
        <span class="k8s-hero-chip">集群 · {{ selectedCluster?.name || '未选择' }}</span>
        <span class="k8s-hero-chip">{{ clusterConnectionLabel }}</span>
      </div>
    </section>

    <section class="stats-grid k8s-top-stats">
      <article v-for="card in summaryCards" :key="card.label" class="release-stat-card k8s-summary-strip" :class="card.tone">
    ...
    <section class="panel k8s-context-strip">
      <div class="k8s-context-strip-copy">
        <span class="k8s-context-strip-title">当前操作上下文</span>
        <span class="k8s-context-strip-desc">切换资源视图前先确认集群、连接状态与命名空间范围。</span>
      </div>
      <div class="k8s-context-strip-items">
        <span class="k8s-context-item">{{ activeTabLabel }}</span>
        <span class="k8s-context-item">{{ selectedCluster?.name || '未选择集群' }}</span>
        <span class="k8s-context-item">{{ clusterConnectionLabel }}</span>
        <span v-if="needsNamespace" class="k8s-context-item">命名空间 · {{ selectedNamespaceLabel }}</span>
        <span v-if="summary.pvcs_pending" class="k8s-context-item k8s-context-item--warning">PVC Pending · {{ summary.pvcs_pending }}</span>
      </div>
    </section>

    <section class="panel k8s-nav-panel">
      <div class="k8s-nav-head">
        <span class="k8s-nav-title">资源视图</span>
        <span class="k8s-nav-desc">在同一工作台内切换集群治理、节点、工作负载与配置资源。</span>
      </div>
    ...
    <template v-else-if="selectedClusterId">
      <div class="workbench-card k8s-resource-card">
        ...
        <div class="k8s-resource-layout">
          <aside class="k8s-resource-aside">
            <section class="panel k8s-aside-panel">
              <div class="k8s-aside-head">
                <strong>集群信息</strong>
                <el-tag size="small" :type="selectedClusterConnected ? 'success' : 'info'">{{ clusterConnectionLabel }}</el-tag>
              </div>
              <div class="k8s-aside-kv">
                <div class="k8s-kv">
                  <span>集群</span>
                ...
                <div class="k8s-kv">
                  ...
                  <strong class="k8s-kv-mono">{{ selectedCluster?.api_server || '-' }}</strong>
                </div>
                <div class="k8s-kv">
                  <span>命名空间</span>
                  <strong>{{ needsNamespace ? selectedNamespaceLabel : '无需选择' }}</strong>
                </div>
                <div class="k8s-kv">
                  <span>当前视图</span>
              ...
              <div class="k8s-aside-actions">
                <el-button size="small" plain @click="switchTab('clusters')">集群管理</el-button>
                <el-button size="small" type="primary" plain @click="refreshView">刷新视图</el-button>
              </div>
            </section>
          </aside>

          <div class="k8s-resource-main">
...
const activeTabLabel = computed(() => mainTabs.find(item => item.key === activeTab.value)?.label || '集群管理')
const clusterConnectionLabel = computed(() => {
  if (!selectedCluster.value) return '连接状态 · 未选择'
  return selectedClusterConnected.value ? '连接状态 · 在线' : '连接状态 · 离线'
})
const selectedNamespaceLabel = computed(() => {
  if (!needsNamespace.value) return '不适用'
  return selectedNamespace.value === '_all' ? '全部命名空间' : (selectedNamespace.value || '未选择')
})
...
const summaryCards = computed(() => {
  if (activeTab.value === 'clusters') {
    ...
    return [
      { label: '集群数', value: clusters.value.length, meta: '纳管集群', foot: '控制面板', badge: '总览', tone: 'tone-info' },
      { label: '已连接', value: connected, meta: '可访问集群', foot: '健康状态', badge: '在线', tone: 'tone-success' },
      { label: '离线集群', value: Math.max(clusters.value.length - connected, 0), meta: '待排查目标', foot: '连接检查', badge: '关注', tone: 'tone-warning' },
      { label: '当前集群', value: selectedCluster.value?.name || '未选择', meta: '活动上下文', foot: clusterConnectionLabel.value.replace('连接状态 · ', ''), badge: '上下文', tone: 'tone-primary' },
    ]
  }
  return [
    { label: 'Ready 节点', value: `${effectiveSummary.value.nodes_ready}/${effectiveSummary.value.nodes_total}`, meta: '节点健康度', foot: activeTabLabel.value, badge: '健康', tone: 'tone-info' },
    { label: 'Pod 总数', value: effectiveSummary.value.pods_total, meta: '运行实例', foot: selectedNamespaceLabel.value, badge: '运行', tone: 'tone-success' },
    { label: '异常 Pod', value: effectiveSummary.value.pods_abnormal, meta: '需要处理', foot: '告警巡检', badge: '关注', tone: 'tone-warning' },
    { label: '工作负载', value: effectiveSummary.value.workloads_total, meta: 'Deployment / Job 等', foot: selectedCluster.value?.name || '未选择', badge: '资源', tone: 'tone-primary' },
  ]
})
...
function initExecTerminal() {
  ...
  execFitAddon.fit()
  execTerminal.writeln('\x1b[1;36mKuberPilot Pod Terminal\x1b[0m')
...
.k8s-summary-strip.tone-success::before {
  background: rgba(16, 185, 129, 0.82);
}

.k8s-summary-strip.tone-warning::before {
  background: rgba(245, 158, 11, 0.86);
}

.k8s-summary-strip.tone-primary::before,
.k8s-summary-strip.tone-info::before {
  background: rgba(37, 99, 235, 0.82);
}

.k8s-summary-strip .stat-card-head,
.k8s-summary-strip .stat-card-foot {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.k8s-summary-strip .stat-card-label,
.k8s-summary-strip .stat-card-foot span {
  color: #64748b;
  font-size: 11px;
}

.k8s-summary-strip .stat-card-badge {
  display: inline-flex;
  align-items: center;
  min-height: 20px;
  padding: 0 7px;
...
.k8s-summary-strip .stat-card-value {
  margin-top: 12px;
  color: #0f172a;
  font-size: 24px;
  font-weight: 650;
...
.k8s-context-strip-title {
  color: #0f172a;
  font-size: 13px;
  font-weight: 650;
}

.k8s-context-strip-desc {
  color: #64748b;
  font-size: 11px;
}

.k8s-context-strip-items {
  display: flex;
  align-items: center;
  justify-content: flex-end;
  gap: 8px;
...
.k8s-page-shell :deep(.k8s-main-tabs.neo-tabs) {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 6px;
  padding: 4px;
...
.k8s-page-shell :deep(.workbench-toolbar-left .el-input__wrapper) {
  min-height: 34px;
  border-radius: 10px;
...
.k8s-page-shell :deep(.workbench-toolbar-left .el-input__wrapper:hover) {
  box-shadow: 0 0 0 1px rgba(15, 23, 42, 0.12) inset;
}

.k8s-page-shell :deep(.workbench-toolbar-left .el-input__inner) {
  font-size: 12px;
}

.k8s-page-shell :deep(.workbench-card-actions .el-button),
.k8s-page-shell :deep(.filter-refresh-btn.el-button) {
  min-height: 32px;
  border-radius: 10px;
  padding: 0 11px;
}

.k8s-page-shell :deep(.workbench-card-actions .el-button:not(.el-button--primary)),
.k8s-page-shell :deep(.filter-refresh-btn.el-button:not(.el-button--primary)) {
  background: rgba(255, 255, 255, 0.94);
  border-color: rgba(15, 23, 42, 0.08);
  color: #0f172a;
}

.k8s-page-shell :deep(.workbench-card-actions .el-button:not(.el-button--primary):hover),
.k8s-page-shell :deep(.filter-refresh-btn.el-button:not(.el-button--primary):hover) {
  border-color: rgba(15, 23, 42, 0.14);
  background: #fff;
}

.k8s-page-shell :deep(.workbench-toolbar-right .el-tag) {
  min-height: 26px;
  border-radius: 999px;
  font-size: 11px;
  font-weight: 600;
