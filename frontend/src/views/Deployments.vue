  ...
  <EntityListShell class="release-page" :title="pageTitle" :description="pageSubtitle" eyebrow="Release Console">
    ...
    <div v-if="!isFlowMode" class="workbench-card release-content-card">
      ...
      <div class="workbench-toolbar workbench-toolbar--history release-filter-bar">
        ...
        <div class="workbench-toolbar-right">
          <el-tag size="small" effect="plain" class="release-toolbar-tag">共 {{ filteredDeployments.length }} 条</el-tag>
      ...
      <el-table v-loading="loading" :data="filteredDeployments" stripe style="width: 100%" class="release-orders-table">
              ...
              <template #default="{ row }">
                <div class="app-cell">
                  <div class="app-cell-title">
                    ...
                    <el-tag v-if="row.is_current" size="small" type="success" class="release-inline-tag">当前生效</el-tag>
                  </div>
            ...
            <el-table-column label="策略" min-width="150">
              <template #default="{ row }">
                <div class="strategy-cell">
                  <el-tag :type="strategyTagType(row.release_strategy)" size="small" class="release-inline-tag">{{ row.release_strategy_display }}</el-tag>
              ...
              <template #default="{ row }">
                <div class="scope-cell">
                  <div class="scope-cell__env">
                    <el-tag size="small" :type="envTagType(row.environment)" class="release-inline-tag">{{ row.environment_display || '-' }}</el-tag>
            ...
            <el-table-column label="审批" width="90">
              <template #default="{ row }">
                <el-tag size="small" :type="approvalTagType(row.approval_status)" class="release-inline-tag release-status-tag">{{ row.approval_status_display }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="状态" width="90">
              <template #default="{ row }">
                <el-tag size="small" :type="statusTagType(row.status)" class="release-inline-tag release-status-tag">{{ row.status_display }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="240" fixed="right" align="left" header-align="left" class-name="release-actions-column">
              <template #default="{ row }">
                <div class="release-row-actions">
                  <el-button v-if="canApproveDeployments && row.approval_status === 'pending'" link type="success" size="small" class="release-row-action" @click="openApprovalDialog(row, 'approve')">通过</el-button>
                  <el-button v-if="canApproveDeployments && row.approval_status === 'pending'" link type="danger" size="small" class="release-row-action" @click="openApprovalDialog(row, 'reject')">驳回</el-button>
                  <el-button v-if="canManageDeployments && row.can_advance_batch" link type="warning" size="small" class="release-row-action" @click="handleAdvanceBatch(row)">推进批次</el-button>
                  <el-button v-if="canManageDeployments && row.approval_status !== 'pending' && row.status !== 'deploying'" link type="warning" size="small" class="release-row-action" @click="handleRerun(row)">重新执行</el-button>
                  <el-button v-if="canManageDeployments && row.can_rollback" link type="warning" size="small" class="release-row-action" @click="handleRollback(row)">回滚</el-button>
                  <el-button v-if="canManageDeployments && row.is_current && row.status === 'stopped'" link type="success" size="small" class="release-row-action" @click="handleStart(row)">启动</el-button>
                  <el-button v-if="canManageDeployments && row.is_current && ['running', 'stopped', 'failed'].includes(row.status)" link type="danger" size="small" class="release-row-action" @click="handleRemove(row)">下线</el-button>
                  <el-button link type="info" size="small" class="release-row-action" @click="viewDetail(row)">详情</el-button>
                </div>
              </template>
            </el-table-column>
      </el-table>
    ...
    <div v-else class="workbench-card release-content-card">
      ...
      <div class="workbench-toolbar workbench-toolbar--history release-filter-bar">
        ...
        <div class="workbench-toolbar-right">
          <el-tag size="small" effect="plain" class="release-toolbar-tag">共 {{ filteredFlows.length }} 条</el-tag>
          <el-button class="filter-refresh-btn" @click="fetchFlows">
            <el-icon><RefreshRight /></el-icon>
            刷新
          </el-button>
        </div>
      ...
      <el-table v-loading="flowLoading" :data="filteredFlows" stripe style="width: 100%" class="release-flows-table">
            <el-table-column label="流程名称" min-width="220">
              <template #default="{ row }">
                <div class="flow-name-cell">
                  <strong>{{ row.name }}</strong>
                  <span>{{ row.description || '用于统一维护发布审批路径与节点顺序。' }}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="适用范围" width="130">
              <template #default="{ row }">
                <el-tag size="small" :type="row.environment ? envTagType(row.environment) : 'info'" class="release-inline-tag">{{ row.environment_display }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="状态" width="100">
              <template #default="{ row }">
                <el-tag size="small" :type="row.is_active ? 'success' : 'info'" class="release-inline-tag release-status-tag">{{ row.is_active ? '启用中' : '停用' }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="180" fixed="right" class-name="release-actions-column">
              <template #default="{ row }">
                <div class="release-row-actions">
                  <el-button v-if="canManageDeployments" link type="primary" size="small" class="release-row-action" @click="openFlowDialog(row)">编辑</el-button>
                  <el-button v-if="canManageDeployments" link type="danger" size="small" class="release-row-action" @click="handleDeleteFlow(row)">删除</el-button>
                </div>
              </template>
            </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="releaseDialogVisible" title="新建发布单" width="960px" append-to-body destroy-on-close class="release-dialog">
      <el-form :model="releaseForm" label-width="110px" class="release-form-grid release-dialog-form">
        ...
        <el-form-item label="发布模式" required>
          <el-radio-group v-model="releaseForm.deploy_mode">
            <el-radio-button value="docker_compose">Docker 环境</el-radio-button>
            <el-radio-button value="k8s">K8s 集群</el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="发布策略" class="span-2">
          <el-radio-group v-model="releaseForm.release_strategy">
            <el-radio-button value="standard">标准发布</el-radio-button>
            <el-radio-button value="canary">灰度发布</el-radio-button>
            <el-radio-button value="batch">批次发布</el-radio-button>
          </el-radio-group>
        </el-form-item>
    ...
    <el-dialog v-model="approvalVisible" :title="approvalAction === 'approve' ? '审批通过' : '驳回发布'" width="520px" append-to-body destroy-on-close class="release-dialog release-dialog--compact">
      <el-form :model="approvalForm" label-width="96px" class="release-dialog-form">
    ...
    <el-dialog v-model="flowDialogVisible" :title="flowEditingId ? '编辑审批流' : '新建审批流'" width="920px" append-to-body destroy-on-close class="release-dialog">
      <el-form :model="flowForm" label-width="100px" class="release-dialog-form">
    ...
    <el-dialog v-model="detailVisible" title="发布详情" width="920px" append-to-body destroy-on-close class="release-dialog release-detail-dialog">
      <template v-if="detailItem">
        ...
        <template v-else-if="detailStatus">
          ...
          <div class="status-meta">
            <el-tag size="small" :type="strategyTagType(detailStatus.release_strategy)" class="release-inline-tag">{{ detailStatus.release_strategy_display || '标准发布' }}</el-tag>
        ...
        <div class="approval-steps">
          <div v-if="detailItem.approval_steps?.length" v-for="step in detailItem.approval_steps" :key="step.id" class="approval-step-card">
            <div class="approval-step-top">
              ...
              <el-tag size="small" :type="stepStatusTagType(step.status)" class="release-inline-tag release-status-tag">{{ step.status_display }}</el-tag>
            </div>
...
const currentModeLabel = computed(() => isFlowMode.value ? '审批流工作台' : '发布工单工作台')
const currentSummaryLabel = computed(() => {
  if (isFlowMode.value) {
    return {
      ...
      scoped: '覆盖多工单',
      dense: '多节点流程',
    }[activeFlowSummaryKey.value] || '全部审批流'
  }
  return {
    all: '全部发布单',
    pending: '待审批',
    running: '运行中',
    failed: '执行失败',
  }[activeSummaryKey.value] || '全部发布单'
})
const currentScopeLabel = computed(() => {
  if (isFlowMode.value) {
    return flowEnvFilter.value
      ? `环境 · ${environmentOptions.find(item => item.value === flowEnvFilter.value)?.label || flowEnvFilter.value}`
      : '环境 · 全部'
  }
  return envFilter.value
    ? `环境 · ${environmentOptions.find(item => item.value === envFilter.value)?.label || envFilter.value}`
    : '环境 · 全部'
})
    ...
    return [
      {
        key: 'all',
        label: '审批流总数',
        value: flowSummary.value.total,
        meta: '流程定义',
        foot: '工作台总览',
        badge: '全部',
        tone: 'tone-info',
        active: activeFlowSummaryKey.value === 'all',
        action: () => applyFlowSummaryFilter('all'),
      },
      {
        key: 'active',
        label: '启用中',
        value: flowSummary.value.active,
        meta: '在线流程',
        foot: '可直接复用',
        badge: '运行',
        tone: 'tone-success',
        active: activeFlowSummaryKey.value === 'active',
        action: () => applyFlowSummaryFilter('active'),
      },
      {
        key: 'scoped',
        label: '覆盖工单类型',
        value: flowSummary.value.ticketScopes,
        meta: '流程覆盖面',
        foot: '范围配置',
        badge: '范围',
        tone: 'tone-warning',
        active: activeFlowSummaryKey.value === 'scoped',
        action: () => applyFlowSummaryFilter('scoped'),
      },
      {
        key: 'dense',
        label: '审批节点数',
        value: flowSummary.value.nodeCount,
        meta: '流程密度',
        foot: '节点编排',
        badge: '节点',
        tone: 'tone-primary',
        active: activeFlowSummaryKey.value === 'dense',
        action: () => applyFlowSummaryFilter('dense'),
      },
    ]
  }
  return [
    {
      ...
      badge: '全部',
      tone: 'tone-info',
      active: activeSummaryKey.value === 'all',
      action: () => applySummaryFilter('all'),
    },
    {
      key: 'pending',
      label: '待审批',
      ...
      badge: '审批',
      tone: 'tone-warning',
      active: activeSummaryKey.value === 'pending',
      action: () => applySummaryFilter('pending'),
    },
    {
      key: 'running',
      label: '运行中',
      ...
      badge: '运行',
      tone: 'tone-success',
      active: activeSummaryKey.value === 'running',
      action: () => applySummaryFilter('running'),
    },
    {
      key: 'failed',
      label: '执行失败',
      ...
      badge: '告警',
      tone: 'tone-danger',
      active: activeSummaryKey.value === 'failed',
      action: () => applySummaryFilter('failed'),
    },
  ]
})
...
const envTagType = (env) => ({ prod: 'danger', test: 'warning', dev: 'info' }[env] || undefined)
const approvalTagType = (status) => ({ pending: 'warning', approved: 'success', rejected: 'danger' }[status] || undefined)
const statusTagType = (status) => ({ pending: 'warning', rejected: 'danger', deploying: 'warning', running: 'success', stopped: 'info', failed: 'danger', removed: 'info' }[status] || undefined)
const stepStatusTagType = (status) => ({ pending: 'warning', approved: 'success', rejected: 'danger' }[status] || undefined)
const strategyTagType = (strategy) => ({ canary: 'warning', batch: 'success' }[strategy] || undefined)
...
<style scoped>
.release-page { display: flex; flex-direction: column; gap: 12px; }
.panel {
...
.hero h2 { color: #0f172a; font-size: 22px; margin: 0; }
.subtitle { color: #475569; margin: 8px 0 0; max-width: 620px; }
.release-hero-copy { display: flex; flex-direction: column; gap: 8px; min-width: 0; }
...
.release-hero-title-block { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.inline-subtitle { margin: 0; max-width: none; font-size: 12px; line-height: 1.45; }
.release-header-icon {
  width: 38px; height: 38px; border-radius: 12px; display: inline-flex; align-items: center; justify-content: center;
  font-size: 18px; color: #245bdb; background: linear-gradient(180deg,#f3f7ff 0%,#ebf2ff 100%);
  border: 1px solid rgba(36,91,219,.12); box-shadow: inset 0 1px 0 rgba(255,255,255,.8);
}
.release-hero-meta { display: flex; align-items: center; justify-content: flex-end; gap: 8px; flex-wrap: wrap; }
.release-hero-chip {
  display: inline-flex; align-items: center; min-height: 28px; padding: 0 10px; border-radius: 999px;
  border: 1px solid rgba(15,23,42,.08); background: rgba(255,255,255,.78); color: #475569; font-size: 11px; font-weight: 600;
}
.release-top-stats { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; }
.release-summary-strip {
  min-width: 0; padding: 10px 12px 10px; border-radius: 14px; border: 1px solid rgba(15,23,42,.08); background: #fff;
  box-shadow: none; position: relative; overflow: hidden; text-align: left; cursor: pointer; transition: all .18s ease;
}
.release-summary-strip::before {
  content: ''; position: absolute; top: 0; left: 12px; right: 12px; height: 2px; border-radius: 999px; background: rgba(15,23,42,.06);
}
.release-summary-strip.tone-success::before { background: rgba(16,185,129,.82); }
.release-summary-strip.tone-warning::before { background: rgba(245,158,11,.86); }
.release-summary-strip.tone-danger::before { background: rgba(239,68,68,.82); }
.release-summary-strip.tone-primary::before,
.release-summary-strip.tone-info::before { background: rgba(37,99,235,.82); }
.release-summary-strip:hover {
  border-color: rgba(15,23,42,.14);
  box-shadow: 0 2px 8px rgba(15,23,42,.05);
}
.release-summary-strip.is-active {
  border-color: rgba(37,99,235,.2);
  background: linear-gradient(180deg, #f7faff 0%, #ffffff 100%);
  box-shadow: inset 0 0 0 1px rgba(37,99,235,.06);
}
.release-summary-strip .stat-card-head,
.release-summary-strip .stat-card-foot { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.release-summary-strip .stat-card-label,
.release-summary-strip .stat-card-foot span { color: #64748b; font-size: 11px; }
.release-summary-strip .stat-card-badge {
  display: inline-flex; align-items: center; min-height: 18px; padding: 0 7px; border-radius: 999px;
  background: rgba(15,23,42,.03); color: #64748b; font-size: 10px; font-weight: 700;
}
.release-summary-strip .stat-card-value {
  margin-top: 10px; color: #0f172a; font-size: 22px; font-weight: 650; letter-spacing: -.03em; line-height: 1.05;
}
.release-summary-strip .stat-card-foot {
  margin-top: 7px; padding-top: 7px; border-top: 1px solid rgba(15,23,42,.05);
}
.release-context-strip { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.release-context-strip-copy { display: flex; flex-direction: column; gap: 3px; min-width: 0; }
.release-context-strip-title { color: #0f172a; font-size: 13px; font-weight: 650; }
.release-context-strip-desc { color: #64748b; font-size: 11px; }
.release-context-strip-items { display: flex; align-items: center; justify-content: flex-end; gap: 8px; flex-wrap: wrap; }
.release-context-item {
  display: inline-flex; align-items: center; min-height: 28px; padding: 0 10px; border-radius: 999px;
  background: rgba(248,250,252,.95); border: 1px solid rgba(148,163,184,.16); color: #475569; font-size: 11px; font-weight: 600;
}
...
.app-cell,.stack-cell,.strategy-cell { display:flex; flex-direction:column; gap:4px; }
.app-cell-title { display:flex; align-items:center; gap:6px; }
.app-name { font-weight:600; color: #0f172a; }
.tag-line { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
.scope-cell { display:flex; flex-direction:column; gap:4px; min-width:0; }
.scope-cell__env { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
.scope-cell__mode { font-size:11px; color:#64748b; }
.scope-cell__system { font-size:12px; color:#0f172a; font-weight:600; line-height:1.35; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.detail-target-cell { min-width:0; }
:deep(.release-content-card .section-toolbar) {
  padding-bottom: 6px;
  border-bottom: 1px solid rgba(15,23,42,.05);
}
:deep(.release-orders-table .release-actions-column .cell),
:deep(.release-flows-table .release-actions-column .cell) { display:flex; justify-content:flex-start; padding-left:8px; padding-right:0; }
...
:deep(.release-flows-table .el-table__fixed-right::before) { left:72px; right:auto; width:8px; background:linear-gradient(90deg,rgba(15,23,42,.1),rgba(15,23,42,0)); }
:deep(.release-filter-bar .workbench-toolbar-left) { gap: 8px; }
:deep(.release-filter-bar .workbench-toolbar-right) { gap: 8px; }
:deep(.workbench-toolbar-left .el-input__wrapper),
:deep(.workbench-toolbar-left .el-select__wrapper) {
  min-height: 32px; border-radius: 10px; background: rgba(255,255,255,.94); box-shadow: 0 0 0 1px rgba(15,23,42,.08) inset;
}
:deep(.workbench-toolbar-left .el-input__wrapper:hover),
:deep(.workbench-toolbar-left .el-select__wrapper:hover) { box-shadow: 0 0 0 1px rgba(15,23,42,.12) inset; }
:deep(.workbench-toolbar-left .el-input__inner),
:deep(.workbench-toolbar-left .el-select__selected-item) { font-size: 12px; }
:deep(.workbench-card-actions .el-button),
:deep(.filter-refresh-btn.el-button) { min-height: 32px; border-radius: 10px; padding: 0 11px; }
:deep(.filter-refresh-btn.el-button:not(.el-button--primary)) {
  background: rgba(255,255,255,.94); border-color: rgba(15,23,42,.08); color: #0f172a;
}
:deep(.filter-refresh-btn.el-button:not(.el-button--primary):hover) { border-color: rgba(15,23,42,.14); background: #fff; }
:deep(.workbench-toolbar-right .el-tag) { min-height: 24px; border-radius: 999px; font-size: 11px; font-weight: 600; }
:deep(.release-filter-bar .el-switch) {
  --el-switch-on-color: #0f172a;
  --el-switch-off-color: rgba(148,163,184,.32);
}
:deep(.release-orders-table .el-table),
:deep(.release-content-card .el-table),
:deep(.release-flows-table .el-table) {
  --el-table-border-color: rgba(15,23,42,.08);
  --el-table-header-bg-color: #f8fafc;
  --el-table-row-hover-bg-color: #f8fbff;
  border-radius: 14px; overflow: hidden;
}
:deep(.release-content-card .el-table th.el-table__cell) { height: 40px; padding: 0; }
:deep(.release-content-card .el-table th.el-table__cell > .cell) { font-size: 12px; font-weight: 700; color: #64748b; }
:deep(.release-content-card .el-table .el-table__cell) { padding: 7px 0; }
:deep(.release-content-card .el-table .cell) { line-height: 1.45; }
.release-toolbar-tag {
...
:deep(.release-row-action.el-button) {
  min-height: 24px;
  padding: 0;
  font-size: 12px;
}
...
.node-preview { display:flex; flex-wrap:wrap; gap:6px; }
.node-chip {
  display:inline-flex;
  align-items:center;
  gap:4px;
...
.release-form-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:0 18px; }
.release-form-grid :deep(.span-2) { grid-column: span 2; }
.flow-nodes-card {
  margin-top:8px;
  border:1px solid rgba(15,23,42,.08);
  border-radius:16px;
...
.flow-nodes-header { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:8px; }
.section-title { font-size:15px; font-weight:600; color:var(--text-primary); }
.status-meta { display:flex; flex-wrap:wrap; align-items:center; gap:10px; margin-bottom:8px; }
.status-message { margin-bottom:8px; }
.approval-steps { display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:10px; }
.approval-step-card {
  padding:12px;
  border-radius:14px;
  border:1px solid rgba(15,23,42,.08);
...
.approval-step-top { display:flex; justify-content:space-between; align-items:center; gap:10px; }
.approval-step-title { font-weight:600; color: #0f172a; }
...
:deep(.release-dialog .el-dialog) {
  border: 1px solid rgba(15,23,42,.08);
  border-radius: 18px;
  box-shadow: 0 20px 48px rgba(15,23,42,.16);
  overflow: hidden;
}
:deep(.release-dialog .el-dialog__header) {
  margin-right: 0;
  padding: 18px 20px 14px;
  border-bottom: 1px solid rgba(15,23,42,.06);
  background: linear-gradient(180deg, rgba(255,255,255,.98) 0%, rgba(248,250,252,.96) 100%);
}
:deep(.release-dialog .el-dialog__title) {
  color: #0f172a;
  font-size: 16px;
  font-weight: 650;
  letter-spacing: -.01em;
}
:deep(.release-dialog .el-dialog__body) {
  padding: 18px 20px;
  background: #fff;
}
:deep(.release-dialog .el-dialog__footer) {
  padding: 14px 20px 18px;
  border-top: 1px solid rgba(15,23,42,.06);
  background: rgba(248,250,252,.72);
}
:deep(.release-dialog .el-form-item) {
  margin-bottom: 16px;
}
:deep(.release-dialog-form .el-form-item__label) {
  color: #475569;
  font-size: 12px;
  font-weight: 600;
}
:deep(.release-dialog-form .el-input__wrapper),
:deep(.release-dialog-form .el-textarea__inner),
:deep(.release-dialog-form .el-select__wrapper),
:deep(.release-dialog-form .el-input-number) {
  border-radius: 12px;
}
:deep(.release-dialog-form .el-input__wrapper),
:deep(.release-dialog-form .el-select__wrapper) {
  ...
  box-shadow: 0 0 0 1px rgba(15,23,42,.08) inset !important;
}
:deep(.release-dialog-form .el-input__wrapper:hover),
:deep(.release-dialog-form .el-select__wrapper:hover) {
...
:deep(.release-dialog-form .el-input-number .el-input__wrapper) {
  box-shadow: none !important;
}
:deep(.release-dialog-form .el-radio-group) {
  gap: 8px;
  display: inline-flex;
  flex-wrap: wrap;
}
:deep(.release-dialog-form .el-radio-button__inner) {
  min-height: 34px;
  padding: 0 14px;
...
:deep(.release-dialog-form .el-checkbox-group) {
  display: flex;
  flex-wrap: wrap;
  gap: 8px 12px;
}
:deep(.release-dialog-form .el-switch) {
  --el-switch-on-color: #2563eb;
  --el-switch-off-color: rgba(148,163,184,.32);
}
:deep(.release-detail-dialog .el-descriptions) {
  border-radius: 14px;
  overflow: hidden;
}
:deep(.release-detail-dialog .el-descriptions__label) {
  color: #64748b;
  font-weight: 600;
}
:deep(.release-detail-dialog .el-descriptions__content) {
  color: #0f172a;
}
:deep(.release-detail-dialog .el-divider__text) {
  color: #0f172a;
  font-weight: 650;
}
...
@media (max-width: 1200px) {
  .release-top-stats { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .release-context-strip { align-items: flex-start; flex-direction: column; }
  .release-context-strip-items { width: 100%; justify-content: flex-start; }
  .release-form-grid { grid-template-columns:1fr; }
  .release-form-grid :deep(.span-2) { grid-column:span 1; }
  .release-filter-bar { align-items: flex-start; }
}
@media (max-width: 768px) {
  .hero { flex-direction: column; align-items: flex-start; }
  .release-hero-meta { justify-content: flex-start; width: 100%; }
  .release-top-stats { grid-template-columns: 1fr; }
}
