<template>
  <div class="fade-in aiops-config-page workbench-page-shell">
    ...
    <div class="stats-grid aiops-config-stats">
      <div v-for="card in configSummaryCards" :key="card.label" class="stat-card release-stat-card" :class="card.tone">
    ...
    <section class="workbench-inline-tip--panel aiops-config-tip">
      <div class="tip-panel-head">
        <strong>智能体治理上下文</strong>
        <span>当前聚焦 {{ activeTabLabel }}，建议先确认默认提供商、运行开关和安全边界，再逐步维护 Action、Skill、MCP 与模型提供商。</span>
      </div>
      <div class="tip-panel-list">
        <div class="tip-panel-item">策略生效状态：{{ configForm.is_enabled ? '智能助手已启用，可直接服务聊天入口和自动化流程。' : '智能助手当前关闭，建议先确认默认策略与运行模式。' }}</div>
        <div class="tip-panel-item">Action {{ actionOverview.total }} 个、Skill {{ skillOverview.total }} 个，决定智能体可调用的执行与推理能力边界。</div>
        <div class="tip-panel-item">已登记 MCP {{ mcpServers.length }} 个、模型提供商 {{ providers.length }} 个，建议优先排查运行就绪项。</div>
        <div class="tip-panel-item">这些配置会被聊天入口、协同任务、Runbook 与复盘知识共用，适合在首屏完成快速巡检。</div>
      </div>
    </section>
      ...
      <template v-else-if="activeTab === 'orchestration'">
        ...
        <div v-if="canViewMcpServer" class="mcp-server-panel">
          <div class="section-toolbar audit-toolbar">
            <div class="toolbar-head">
              <span class="toolbar-title">KuberPilot 对外 MCP Server</span>
...
import {
  ...
  providerRuntimeTagType,
  skillRecommendedToolCount,
  skillRecommendedTools,
  skillRiskTagType,
} from './aiops-config/helpers'
import {
  archiveAIOpsRunbook,
...
const activeTabLabel = computed(() => ({
  ...
  orchestration: '拓展能力',
  providers: '模型提供商',
}[activeTab.value] || '智能体策略'))
const configSummaryCards = computed(() => [
  {
    label: '运行状态',
    value: configForm.is_enabled ? '已启用' : '未启用',
    desc: '控制聊天入口和智能助手整体可用性',
    tone: configForm.is_enabled ? 'success-card' : 'warning-card',
  },
  {
    label: 'Action 能力',
    value: actionOverview.value.total,
    desc: `当前可用 ${actionOverview.value.available} 个，执行型 ${actionOverview.value.execute} 个`,
    tone: '',
  },
  {
    label: 'Skill 能力',
    value: skillOverview.value.total,
    desc: `内置 ${skillOverview.value.builtin} 个，已启用 ${skillOverview.value.enabled} 个`,
    tone: 'success-card',
  },
  {
    label: 'MCP / 提供商',
    value: `${mcpServers.value.length} / ${providers.value.length}`,
    desc: '统一维护工具接入和模型运行入口',
    tone: '',
  },
])
...
function resetMcpForm() {
  Object.assign(mcpForm, createMcpForm())
  mcpToolDiagnostics.value = []
}

function resetSkillForm() {
  Object.assign(skillForm, createSkillForm())
}

function resetA2AForm() {
  Object.assign(a2aForm, createA2AForm())
}

function resetRunbookForm() {
  Object.assign(runbookForm, createRunbookForm())
}
...
async function loadAll() {
  ...
  try {
    ...
    providers.value = normalizeProviderList(providerData, providerPresets.value)
...
function openProviderDialog(row) {
  ...
  if (row) {
    Object.assign(providerForm, row, { api_key: '' })
      selectedProviderPreset.value = detectProviderPreset(row, providerPresets.value)
...
async function saveSkill() {
  ...
  try {
    ...
    if (!outputContract || Array.isArray(outputContract) || typeof outputContract !== 'object') {
      ...
      const { output_contract_text, id, is_builtin, ...restSkillForm } = skillForm
      const payload = {
        ...restSkillForm,
