...
async function fetchBootstrap() {
  ...
  try {
    ...
    bootstrap.value = { enabled: false, permissions: {}, suggested_questions: [], runtime: {} }
  } finally {
...
<style scoped>
.aiops-widget{position:fixed;right:18px;bottom:18px;z-index:80}
...
.aiops-fab{position:relative;z-index:81;display:flex;align-items:center;gap:8px;min-width:120px;height:52px;padding:6px 11px 6px 6px;border:none;border-radius:999px;background:linear-gradient(135deg,#ffffff 0%,#f7fbff 100%);box-shadow:0 10px 20px rgba(59,130,246,.11);cursor:grab;border:1px solid #bdd5fb;transition:transform .18s ease,box-shadow .18s ease;touch-action:none;user-select:none}
...
.aiops-fab-core{position:relative;display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:18px;background:linear-gradient(145deg,#eef5ff 0%,#f6fbff 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.96),0 8px 16px rgba(59,130,246,.08)}
.aiops-fab-avatar{width:27px;height:27px;display:block}
.aiops-fab-label{display:flex;flex-direction:column;align-items:flex-start;line-height:1.1;position:relative;z-index:1}
.aiops-fab-label strong{font-size:12px;color:#0f172a}
.aiops-fab-label small{margin-top:2px;font-size:10px;color:#64748b}
.aiops-fab-dot{position:absolute;top:8px;right:9px;width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 3px rgba(255,255,255,.96),0 0 0 5px rgba(34,197,94,.1)}
.aiops-panel{--aiops-session-width:260px;--aiops-chat-base-width:836px;position:absolute;right:18px;bottom:74px;width:min(1096px,calc(100vw - var(--sidebar-width,188px) - 40px));max-width:calc(100vw - var(--sidebar-width,188px) - 40px);height:min(800px,calc(100vh - 104px));display:flex;flex-direction:column;background:linear-gradient(180deg,#fff 0%,#f8fbff 100%);border:1px solid #dbe4f0;border-radius:24px;box-shadow:0 26px 56px rgba(15,23,42,.18);overflow:hidden}
...
@media (max-width: 920px){
  ...
  .aiops-panel{right:12px;bottom:72px;width:min(720px,calc(100vw - var(--sidebar-width,188px) - 24px));max-width:calc(100vw - var(--sidebar-width,188px) - 24px);height:min(86vh,calc(100vh - 100px))}
