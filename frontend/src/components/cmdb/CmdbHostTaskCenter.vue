<template>
  <div class="task-center-page">
    <section class="glass-card task-view-panel">
      <div class="task-view-head">
        <div class="task-view-copy">
          <span class="task-view-title">功能视图</span>
          <span class="task-view-desc">在同一工作台里切换任务历史、即时下发和模板治理，保持统一的操作节奏与信息密度。</span>
        </div>
        <div class="task-view-meta">
          <span class="task-view-chip">当前视图 · {{ activeTabMeta.label }}</span>
          <span class="task-view-chip">总任务 · {{ taskStats.total || 0 }}</span>
          <span class="task-view-chip">执行中 · {{ taskStats.running || 0 }}</span>
        </div>
      </div>
    ...
    <section class="task-metric-grid">
      <button
        v-for="card in workbenchSummaryCards"
        :key="card.key"
        type="button"
        class="task-metric-card"
        :class="[card.tone, { 'is-active': card.active }]"
        @click="handleSummaryCardClick(card)"
      >
        <div class="task-metric-card__head">
          <span class="task-metric-card__label">{{ card.label }}</span>
          <span class="task-metric-card__badge">{{ card.badge }}</span>
        </div>
        <div class="task-metric-card__value">{{ card.value }}</div>
        <div class="task-metric-card__foot">
          <span>{{ card.meta }}</span>
          <span>{{ card.foot }}</span>
        </div>
      </button>
    ...
    <section class="glass-card task-context-strip">
      <div class="task-context-copy">
        <span class="task-context-title">当前操作上下文</span>
        <span class="task-context-desc">先确认视图、筛选范围和执行对象，再进入历史列表、模板治理或即时下发区。</span>
      </div>
      <div class="task-context-items">
        <span v-for="item in taskContextItems" :key="item" class="task-context-item">{{ item }}</span>
      </div>
    </section>

    <template v-if="activeTab === 'dispatch'">
      <div class="composer-grid">
        <div class="glass-card side-stack dispatch-preset-panel">
        ...
        <div class="glass-card dispatch-form-panel">
          <div class="card-head">
          ...
          <el-form :model="taskForm" label-width="88px" class="task-form task-dispatch-form">
            <div class="form-row">
              <el-form-item :label="ui.taskName" class="form-col wide">
                <el-input v-model="taskForm.name" size="small" :placeholder="ui.taskNamePlaceholder" />
              </el-form-item>
            </div>
            <div class="form-row">
              <el-form-item :label="ui.executionType" class="form-col">
                <el-select v-model="executionKind" size="small" style="width: 100%" @change="applyExecutionKind">
              ...
              <el-form-item :label="ui.executionMode" class="form-col">
                <el-select v-model="taskForm.execution_mode" size="small" style="width: 100%" :disabled="taskForm.task_type === 'run_playbook' || taskForm.target_type === 'k8s'">
            ...
            <el-form-item :label="ui.taskDesc">
              <el-input v-model="taskForm.description" size="small" :placeholder="ui.taskDescPlaceholder" />
            </el-form-item>
            <div v-if="taskForm.target_type === 'host' && taskForm.task_type === 'run_command'" class="dispatch-inline-strip">
              <div class="dispatch-inline-item dispatch-inline-item--hint">
                <span class="dispatch-inline-label">{{ ui.modeHint }}</span>
                <div class="detail-kv compact-kv">{{ executionModeHint(taskForm.execution_mode, taskForm.task_type) }}</div>
              </div>
              <div class="dispatch-inline-item dispatch-inline-item--input">
                <span class="dispatch-inline-label">{{ ui.timeout }}</span>
                <el-input-number v-model="taskForm.timeout_seconds" size="small" :min="5" :max="120" style="width: 100%" />
              </div>
              <div class="dispatch-inline-item dispatch-inline-item--radio">
                <span class="dispatch-inline-label">{{ ui.strategy }}</span>
                <el-radio-group v-model="taskForm.execution_strategy" size="small">
                  <el-radio value="continue">{{ ui.continueOnError }}</el-radio>
                  <el-radio value="stop_on_error">{{ ui.stopOnError }}</el-radio>
                </el-radio-group>
              </div>
            </div>
            <div v-else-if="taskForm.target_type === 'host' && taskForm.task_type === 'run_playbook'" class="dispatch-inline-strip">
              <div class="dispatch-inline-item dispatch-inline-item--input">
                <span class="dispatch-inline-label">{{ ui.playbookFileName }}</span>
                <el-input v-model="taskForm.payload.playbook_name" size="small" :placeholder="ui.playbookNamePlaceholder" />
              </div>
              <div class="dispatch-inline-item dispatch-inline-item--input dispatch-inline-item--narrow">
                <span class="dispatch-inline-label">{{ ui.timeout }}</span>
                <el-input-number v-model="taskForm.timeout_seconds" size="small" :min="5" :max="120" style="width: 100%" />
              </div>
              <div class="dispatch-inline-item dispatch-inline-item--radio">
            ...
            <div v-else-if="taskForm.target_type === 'host'" class="dispatch-inline-strip">
              <div class="dispatch-inline-item dispatch-inline-item--hint">
                <span class="dispatch-inline-label">{{ ui.modeHint }}</span>
                <div class="detail-kv compact-kv">{{ executionModeHint(taskForm.execution_mode, taskForm.task_type) }}</div>
              </div>
            ...
            <div v-else-if="taskForm.target_type === 'k8s'" class="dispatch-inline-strip">
              <div class="dispatch-inline-item dispatch-inline-item--hint">
                <span class="dispatch-inline-label">{{ ui.modeHint }}</span>
                <div class="detail-kv compact-kv">{{ executionModeHint(taskForm.execution_mode, taskForm.task_type) }}</div>
              </div>
            ...
            <div v-if="taskForm.task_type === 'k8s_pod_exec'" class="template-payload-stack">
              <el-form-item :label="ui.script">
                <div class="script-editor-shell">
                  <div class="script-editor-head script-editor-head--meta">
                    <span>K8s</span>
            ...
            <div class="toolbar toolbar--targets">
              <div class="toolbar-row toolbar-row-main">
                <el-input v-model="targetFilters.search" size="small" :placeholder="ui.searchHostPlaceholder" clearable style="width: 220px" @keyup.enter="fetchTargets">
                  <template #prefix><el-icon><Search /></el-icon></template>
                </el-input>
                <el-select v-model="targetFilters.environment" size="small" clearable filterable :placeholder="ui.environment" style="width: 140px" @change="handleEnvironmentChange">
                  <el-option v-for="node in envNodes" :key="node.id" :label="node.name" :value="node.id" />
                </el-select>
                <el-select v-model="targetFilters.system" size="small" clearable :placeholder="ui.businessLine" style="width: 140px" :disabled="!targetFilters.environment">
              ...
              <div class="toolbar-row toolbar-row-summary">
                ...
                <div class="selection-strip selection-strip--stats">
                ...
                <div v-if="effectiveHostTargetRefs.length && selectedTargetsExpanded" class="selected-target-panel">
                  ...
                  >
                    <div class="dispatch-target-cell">
                      <strong>{{ item.name }}</strong>
                      <div class="dispatch-target-sub">{{ item.meta }}</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <el-table ref="hostTableRef" :data="availableHosts" v-loading="targetLoading" row-key="id" max-height="280" :empty-text="ui.emptyHosts" class="dispatch-target-table" @selection-change="handleSelectionChange">
              <el-table-column type="selection" width="42" reserve-selection />
              <el-table-column :label="ui.hostResource" min-width="300">
                <template #default="{ row }">
                  <div class="dispatch-target-cell">
                    <strong>{{ row.name || '-' }}</strong>
                    <div class="dispatch-target-sub">{{ [row.ip_address, row.environment_name, row.system_name].filter(Boolean).join(' · ') || '-' }}</div>
                  </div>
                </template>
              </el-table-column>
              <el-table-column :label="ui.status" width="110">
                <template #default="{ row }">
                  <el-tag size="small" :type="hostStatusTagType(row.status)" class="dispatch-status-tag">{{ row.status_display || '-' }}</el-tag>
                </template>
              </el-table-column>
            </el-table>
            </template>
            ...
            <div class="toolbar toolbar--targets">
              <div class="toolbar-row toolbar-row-main">
                <el-input v-model="targetFilters.search" size="small" :placeholder="ui.searchK8sPlaceholder" clearable style="width: 220px" @keyup.enter="fetchTargets">
                  <template #prefix><el-icon><Search /></el-icon></template>
                </el-input>
                <el-select v-model="targetFilters.environment" size="small" clearable filterable :placeholder="ui.environment" style="width: 140px" @change="handleEnvironmentChange">
                  <el-option v-for="node in envNodes" :key="node.id" :label="node.name" :value="node.id" />
              ...
              <div class="toolbar-row toolbar-row-summary">
                ...
                <div class="selection-strip selection-strip--stats">
                  ...
                  <span class="selection-pill muted">{{ ui.inactiveClusters }} {{ selectedK8sStats.inactive }}</span>
                </div>
              </div>
            </div>
            <el-table ref="k8sTargetTableRef" :data="availableK8sTargets" v-loading="targetLoading" row-key="id" max-height="280" :empty-text="ui.emptyK8sTargets" class="dispatch-target-table" @selection-change="handleK8sSelectionChange">
              <el-table-column type="selection" width="42" reserve-selection />
              <el-table-column :label="ui.k8sResource" min-width="320">
                <template #default="{ row }">
                  <div class="dispatch-target-cell">
                    <strong>{{ row.name || '-' }}</strong>
                    <div class="dispatch-target-sub">{{ row.description || '-' }}</div>
                    <div class="dispatch-target-meta">{{ [row.environment_name, row.system_name].filter(Boolean).join(' · ') || '-' }}</div>
                  </div>
                </template>
              </el-table-column>
              <el-table-column :label="ui.status" width="110">
                <template #default="{ row }">
                  <el-tag size="small" :type="k8sStatusTagType(row.status)" class="dispatch-status-tag">{{ row.status_display || '-' }}</el-tag>
                </template>
              </el-table-column>
            </el-table>
            </template>
      ...
      <div class="glass-card template-list-card">
        <div class="section-toolbar">
          <div class="toolbar-head">
            <span class="toolbar-title">{{ ui.templateLibrary }}</span>
            <span class="toolbar-desc">沉淀常用巡检、诊断与变更模板，用统一的筛选和动作密度管理复用资产。</span>
          </div>
          <div class="workbench-card-actions">
            <el-button size="small" type="primary" @click="openTemplateCreateDialog">{{ ui.newTemplate }}</el-button>
            <el-button size="small" :loading="savingTemplate" @click="saveCurrentAsTemplate">{{ ui.saveCurrentConfig }}</el-button>
          </div>
        </div>
        <div class="workbench-toolbar template-toolbar">
    ...
    <template v-else>
      <div class="glass-card history-card">
        <div class="section-toolbar">
          <div class="toolbar-head">
            <span class="toolbar-title">{{ ui.history }}</span>
            <span class="toolbar-desc">沿用统一工作台列表密度，处理筛选、详情、重跑、终止与模板沉淀。</span>
          </div>
          <div class="workbench-card-actions history-actions">
        ...
        <div class="task-source-grid task-source-grid--compact">
          <button
          ...
          >
            ...
            <div class="task-source-card__foot">
              <span>{{ item.desc }}</span>
              <span>{{ taskFilters.trigger_source === item.source && taskFilters.target_type === (item.targetType || '') ? '已筛选' : '点击查看' }}</span>
            </div>
          </button>
        </div>
        <div class="workbench-toolbar history-toolbar">
          <div class="workbench-toolbar-left">
          ...
          <div class="workbench-toolbar-right">
            <el-tag size="small" effect="plain" type="info">共 {{ taskTotal }} 条</el-tag>
    ...
    <el-dialog v-model="templateCreateVisible" class="task-center-dialog task-center-dialog--editor" :title="templateDialogTitle" width="980px" destroy-on-close append-to-body align-center>
      <template #header>
        <div class="task-dialog-header">
          <span class="task-dialog-eyebrow">Template Editor</span>
          <strong>{{ templateDialogTitle }}</strong>
          <span>{{ ui.templateEditorTip }}</span>
        </div>
      </template>
      ...
      <div class="template-editor-layout">
        <div class="template-editor-panel template-editor-main">
          <el-form :model="templateDraft" label-width="92px" class="task-form template-editor-form">
            <div class="template-editor-section">
              <div class="template-editor-section-title">{{ ui.editorBasicInfo }}</div>
              <div class="form-row">
                <el-form-item :label="ui.taskName" class="form-col">
                  <el-input v-model="templateDraft.name" size="small" :placeholder="ui.taskNamePlaceholder" />
                </el-form-item>
                <el-form-item :label="ui.executionType" class="form-col">
                  <el-select v-model="templateExecutionKind" size="small" style="width: 100%" @change="handleTemplateExecutionKindChange">
                    <el-option v-for="option in executionTypeOptions" :key="option.value" :label="option.label" :value="option.value" />
                  </el-select>
                </el-form-item>
              </div>
            ...
            <div class="template-editor-section">
              ...
              <div class="form-row">
                <el-form-item :label="ui.executionMode" class="form-col">
                  <el-select v-model="templateDraft.execution_mode" size="small" style="width: 100%" :disabled="templateDraft.task_type === 'run_playbook' || templateDraft.target_type === 'k8s'">
              ...
              <div v-if="templateDraft.task_type !== 'run_command' && templateDraft.task_type !== 'run_playbook'" class="form-row">
                <el-form-item :label="ui.timeout" class="form-col">
                  <el-input-number v-model="templateDraft.timeout_seconds" size="small" :min="5" :max="120" style="width: 100%" />
                </el-form-item>
                <el-form-item :label="ui.strategy" class="form-col">
                  <el-radio-group v-model="templateDraft.execution_strategy" size="small">
                    <el-radio value="continue">{{ ui.continueOnError }}</el-radio>
                    <el-radio value="stop_on_error">{{ ui.stopOnError }}</el-radio>
                  </el-radio-group>
                </el-form-item>
              ...
              <div v-else-if="templateDraft.task_type === 'run_playbook'" class="template-payload-stack">
                <div class="form-row">
                  <el-form-item :label="ui.playbookName" class="form-col">
                    <el-input v-model="templateDraft.payload.playbook_name" size="small" :placeholder="ui.playbookNamePlaceholder" />
                  </el-form-item>
                  <el-form-item :label="ui.timeout" class="form-col">
                    <el-input-number v-model="templateDraft.timeout_seconds" size="small" :min="5" :max="120" style="width: 100%" />
                  </el-form-item>
    ...
    <el-drawer v-model="templateDetailVisible" class="task-center-drawer" :title="ui.templateDetailTitle" size="42%" append-to-body>
      <template #header>
        <div class="task-drawer-header">
          <span class="task-dialog-eyebrow">Template Detail</span>
          <strong>{{ currentTemplate?.name || ui.templateDetailTitle }}</strong>
          <span>{{ currentTemplate?.description || ui.templateDetailTitle }}</span>
        </div>
      </template>
    ...
    <el-drawer v-model="detailVisible" class="task-center-drawer" :title="ui.detailTitle" size="68%" append-to-body>
      <template #header>
        <div class="task-drawer-header">
          <span class="task-dialog-eyebrow">Execution Detail</span>
          <strong>{{ detailTask?.name || ui.detailTitle }}</strong>
          <span>{{ detailTask?.task_type_display || ui.detailTitle }}</span>
        </div>
      </template>
      <div v-loading="detailLoading" class="task-detail-shell">
        <template v-if="detailTask">
          <div class="detail-heading">
            ...
            <div class="detail-summary compact detail-summary--detail">
          ...
          <div class="task-metric-grid compact detail-metric-grid">
            <div class="task-metric-card detail-metric-card">
              <span class="task-metric-label">{{ ui.targetCount }}</span>
              <strong>{{ detailTask.target_count || 0 }}</strong>
            </div>
            <div class="task-metric-card success detail-metric-card">
              <span class="task-metric-label">{{ ui.success }}</span>
              <strong>{{ detailTask.success_count || 0 }}</strong>
            </div>
            <div class="task-metric-card danger detail-metric-card">
              <span class="task-metric-label">{{ ui.failed }}</span>
              <strong>{{ detailTask.failed_count || 0 }}</strong>
            </div>
            <div class="task-metric-card warning detail-metric-card">
          ...
          <div class="detail-section">
            ...
            <el-table :data="detailTask.executions || []" max-height="520" :empty-text="ui.emptyExecutions" class="detail-execution-table">
              ...
              <el-table-column :label="ui.status" width="88">
                <template #default="{ row }"><el-tag size="small" :type="executionStatusType(row.status)" class="detail-status-tag">{{ row.status_display }}</el-tag></template>
              </el-table-column>
              ...
              <el-table-column :label="ui.output" min-width="560">
                <template #default="{ row }">
                  <div class="output-preview-card" @click="openExecutionOutput(row)">
                    <div class="output-preview-copy">
                      <div class="output-preview-text">{{ previewExecutionOutput(row) }}</div>
                      <div class="output-preview-meta">{{ row.status_display || '-' }} · {{ formatExecutionDuration(row) }}</div>
                    </div>
    ...
    <el-dialog v-model="outputDialogVisible" class="task-center-dialog" :title="outputDialogTitle" width="860px" append-to-body destroy-on-close align-center>
      <template #header>
        <div class="task-dialog-header task-dialog-header--compact">
          <span class="task-dialog-eyebrow">Execution Output</span>
          <strong>{{ outputDialogTitle }}</strong>
          <span>查看原始执行输出与结果回显。</span>
        </div>
      </template>
      <pre class="output-dialog-block">{{ outputDialogContent || '-' }}</pre>
    </el-dialog>
    <el-dialog v-model="k8sLogicVisible" class="task-center-dialog" :title="ui.k8sLogicTitle" width="640px" append-to-body destroy-on-close align-center>
      <template #header>
        <div class="task-dialog-header task-dialog-header--compact">
          <span class="task-dialog-eyebrow">K8s Logic</span>
          <strong>{{ ui.k8sLogicTitle }}</strong>
          <span>统一说明 K8s API 执行模式与资源变更边界。</span>
        </div>
      </template>
...
const activeTabMeta = computed(() => innerTabs.find(item => item.key === activeTab.value) || innerTabs[0])
...
const historyStatusLabel = computed(() => taskStatusOptions.find(item => item.value === taskFilters.value.status)?.label || '全部结果')
const historyTriggerSourceLabel = computed(() => triggerSourceOptions.find(item => item.value === taskFilters.value.trigger_source)?.label || '全部来源')
const historyTargetTypeLabel = computed(() => targetTypeOptions.find(item => item.value === taskFilters.value.target_type)?.label || '全部目标')
const historyExecutionKindLabel = computed(() => taskFilters.value.execution_kind ? executionKindLabel(taskFilters.value.execution_kind) : '全部类型')
const historyRiskLabel = computed(() => riskLevelOptions.find(item => item.value === taskFilters.value.risk_level)?.label || '全部风险')
const templateFilterLabel = computed(() => templateFilters.value.execution_kind ? executionKindLabel(templateFilters.value.execution_kind) : '全部类型')
const currentEnvironmentName = computed(() => envNodes.value.find(item => item.id === targetFilters.value.environment)?.name || '全部环境')
const currentSystemName = computed(() => currentSystemOptions.value.find(item => item.id === targetFilters.value.system)?.name || '全部系统')
const dispatchScopeLabel = computed(() => {
  const scope = [currentEnvironmentName.value]
  if (targetFilters.value.system) scope.push(currentSystemName.value)
  return scope.join(' / ')
})
...
const workbenchSummaryCards = computed(() => {
  const successRate = Number(taskStats.value.success_rate || 0)
  return [
    {
      key: 'history-all',
      label: '任务总量',
      badge: 'History',
      value: taskStats.value.total || 0,
      meta: '统一任务池',
      foot: '查看全部历史任务',
      tone: 'tone-slate',
      active: activeTab.value === 'history' && !taskFilters.value.status,
    },
    {
      key: 'history-running',
      label: '执行中',
      badge: 'Active',
      value: taskStats.value.running || 0,
      meta: '持续刷新中的任务',
      foot: '切到执行中筛选',
      tone: 'tone-blue',
      active: activeTab.value === 'history' && taskFilters.value.status === 'running',
    },
    {
      key: 'library',
      label: '模板沉淀',
      ...
      meta: '可复用执行模板',
      foot: `${templateFilterLabel.value} / ${templates.value.length} 总模板`,
      tone: 'tone-emerald',
    ...
    },
    {
      ...
      label: 'AIOps 待确认',
      badge: 'Dispatch',
      value: taskStats.value.aiops_pending || 0,
      meta: `成功率 ${successRate.toFixed(0)}%`,
      foot: activeTab.value === 'dispatch' ? `已选目标 ${selectedTargetCount.value} 个` : `高风险 ${taskStats.value.high_risk || 0} 个`,
      tone: 'tone-amber',
      active: activeTab.value === 'dispatch',
    },
  ]
})
const taskContextItems = computed(() => {
  const items = [`当前视图 · ${activeTabMeta.value.label}`]
  if (activeTab.value === 'history') {
    items.push(`执行类型 · ${historyExecutionKindLabel.value}`)
    items.push(`结果 · ${historyStatusLabel.value}`)
    items.push(`来源 · ${historyTriggerSourceLabel.value}`)
    items.push(`目标 · ${historyTargetTypeLabel.value}`)
    if (taskFilters.value.risk_level) items.push(`风险 · ${historyRiskLabel.value}`)
    if (taskFilters.value.search.trim()) items.push(`检索 · ${taskFilters.value.search.trim()}`)
    return items
  }
  if (activeTab.value === 'library') {
    items.push(`模板类型 · ${templateFilterLabel.value}`)
    items.push(`模板数 · ${filteredTemplates.value}`)
    if (templateFilters.value.search.trim()) items.push(`检索 · ${templateFilters.value.search.trim()}`)
    return items
  }
  items.push(`执行类型 · ${currentExecutionKindLabel.value}`)
  items.push(`目标类型 · ${taskForm.value.target_type === 'k8s' ? ui.k8sResource : ui.hostResource}`)
  items.push(`资源范围 · ${dispatchScopeLabel.value}`)
  items.push(`已选目标 · ${selectedTargetCount.value} 个`)
  return items
})
function handleSummaryCardClick(card) {
  ...
  taskFilters.value.status = card.key === 'history-running' ? 'running' : ''
  taskPage.value = 1
  fetchTasks()
}
...
function hostStatusTagType(status) {
  const key = String(status || '').trim()
  if (key === 'online') return 'success'
  if (key === 'warning') return 'warning'
  if (key === 'offline') return 'danger'
  return undefined
}
function k8sStatusTagType(status) {
  const key = String(status || '').trim()
  if (key === 'active') return 'success'
  if (key === 'warning') return 'warning'
  if (key === 'inactive') return 'info'
  return undefined
}
...
.dispatch-preset-panel {
  padding: 12px;
}

.dispatch-preset-panel .compact-head {
  margin-bottom: 6px;
}

.dispatch-preset-panel .preset-grid {
  gap: 6px;
}

.dispatch-preset-panel .preset-card {
  padding: 10px 10px;
}

.dispatch-preset-panel .preset-title {
  font-size: 12px;
}

.dispatch-preset-panel .preset-desc {
  margin-top: 3px;
  line-height: 1.4;
}
...
.dispatch-inline-strip {
  display: grid;
  grid-template-columns: minmax(0, 1.3fr) 124px minmax(220px, 0.9fr);
  gap: 8px;
  margin-bottom: 10px;
  padding: 8px 10px;
  border-radius: 12px;
...
.dispatch-inline-item--hint {
  justify-content: center;
}

.dispatch-inline-item--narrow {
  max-width: 124px;
}
...
.dispatch-inline-item .compact-kv {
  padding: 0;
  min-height: 26px;
  display: flex;
  align-items: center;
...
.dispatch-inline-item :deep(.el-radio-group) {
  gap: 8px;
  flex-wrap: wrap;
}

.dispatch-inline-item :deep(.el-radio) {
  margin-right: 0;
}

.task-dispatch-form :deep(.el-form-item__label) {
  color: #64748b;
  font-size: 12px;
  font-weight: 600;
  line-height: 1.2;
...
.task-dispatch-form :deep(.el-input__wrapper),
.task-dispatch-form :deep(.el-select__wrapper),
.task-dispatch-form :deep(.el-textarea__inner),
.task-dispatch-form :deep(.el-input-number) {
  border-radius: 10px;
}
...
.task-center-page :deep(.el-divider) {
  margin: 10px 0 8px;
}
...
.selection-strip--stats {
  justify-content: flex-end;
}

.dispatch-target-table :deep(.el-table__cell) {
  padding: 6px 10px;
}

.dispatch-target-table :deep(.el-table-column--selection .cell) {
  padding-left: 8px;
  padding-right: 0;
}

.dispatch-target-cell {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
...
.selected-target-item .dispatch-target-cell {
  gap: 1px;
}

.selected-target-item .dispatch-target-sub {
  color: #64748b;
}

.selected-target-panel::-webkit-scrollbar {
  width: 6px;
}

.selected-target-panel::-webkit-scrollbar-thumb {
  border-radius: 999px;
...
.task-drawer-header strong {
  color: #0f172a;
  font-size: 16px;
  line-height: 1.25;
}

.task-dialog-header span:last-child,
.task-drawer-header span:last-child {
  color: #64748b;
  font-size: 11px;
  line-height: 1.5;
}

.task-dialog-header--compact strong {
  font-size: 15px;
}
...
.template-editor-form :deep(.el-form-item) {
  margin-bottom: 10px;
}

.template-editor-form :deep(.el-form-item__label) {
  color: #64748b;
  font-size: 12px;
  font-weight: 600;
}
...
.detail-summary--detail .detail-chip {
...
.task-metric-card {
  ...
  border: 1px solid rgba(148, 163, 184, 0.14);
  text-align: left;
  transition: 0.2s ease border-color, 0.2s ease box-shadow, 0.2s ease background, 0.2s ease transform;
}
...
.task-metric-card__head,
.task-metric-card__foot {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.task-metric-card__foot span:last-child {
  text-align: right;
}
...
.task-metric-card.tone-slate {
  background: linear-gradient(180deg, #ffffff 0%, #f8fafc 100%);
}

.task-metric-card.tone-blue {
  background: linear-gradient(180deg, #f8fbff 0%, #eef5ff 100%);
}

.task-metric-card.tone-emerald {
  background: linear-gradient(180deg, #f5fffb 0%, #ecfdf5 100%);
}

.task-metric-card.tone-amber {
...
.detail-execution-table :deep(.el-table__cell) {
  padding: 7px 10px;
}
...
.script-editor-head--meta {
  min-height: 22px;
  padding: 2px 8px;
  background: rgba(15, 23, 42, 0.98);
}

.script-editor-head--meta span {
  color: #94a3b8;
  font-size: 10px;
}

.script-editor-head--meta strong {
  display: inline-flex;
  align-items: center;
  min-height: 18px;
  padding: 0 6px;
...
.task-center-page :deep(.task-center-dialog .el-dialog) {
  border-radius: 20px;
  overflow: hidden;
  border: 1px solid rgba(148, 163, 184, 0.14);
...
.task-center-page :deep(.task-center-dialog .el-dialog__header) {
  margin-right: 0;
...
.task-center-page :deep(.task-center-dialog .el-dialog__body) {
  padding: 14px 16px 16px;
  background: #f8fafc;
}

.task-center-page :deep(.task-center-dialog .el-dialog__footer) {
